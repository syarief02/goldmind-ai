...
//--- Compute ATR(14) using the user's primary InpTimeframe
   MqlRates rArray[];
   ArraySetAsSeries(rArray, true);
   int cpd = CopyRates(_Symbol, InpTimeframe, 0, InpCandleCount, rArray);
   double atr = ComputeATR(rArray, cpd, 14);

//--- Build timeframe string
   string tfStr = TimeframeToString(InpTimeframe);

//--- Build candles structured dictionary
   string candlesObj = "{";
   candlesObj += "\"M5\":[" + m5Candles + "],";
   candlesObj += "\"M15\":[" + m15Candles + "],";
   candlesObj += "\"M30\":[" + m30Candles + "],";
   candlesObj += "\"H1\":[" + h1Candles + "],";
   candlesObj += "\"H4\":[" + h4Candles + "]";
   candlesObj += "}";
...
//+------------------------------------------------------------------+
//| Extract candle formatting into a helper function                  |
//+------------------------------------------------------------------+
string StringBuildCandlesJSON(ENUM_TIMEFRAMES tf, int count)
  {
   MqlRates rates[];
